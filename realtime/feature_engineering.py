...
class FeatureEngineer:
    ...
    def add_technical_indicators(
        ...
        if 'macd' in tech_config:
            ...
            df['macd'] = self.fi.macd(
                df['close'], window_fast=fast, window_slow=slow)
        ...
        if 'stoch_period' in tech_config:
            ...
            df['stoch'] = self.fi.stoch(
                df['high'], df['low'], df['close'], window=period)
            df['stoch_signal'] = self.fi.stoch_signal(
                df['high'], df['low'], df['close'], window=period)
        ...
        if 'bollinger' in tech_config:
            ...
            df['bb_upper'] = self.fi.bollinger_hband(
                df['close'], window=period, window_dev=std_dev)
            df['bb_lower'] = self.fi.bollinger_lband(
                df['close'], window=period, window_dev=std_dev)
    ...
    def add_price_features(
        ...
        if 'volatility_window' in price_config:
            ...
            df['volatility_of_volatility'] = df['volatility'].rolling(
                window=window).std()
        ...
        if 'volume_ratio_window' in price_config and 'volume' in df.columns:
            ...
            df['volume_zscore'] = (
                df['volume'] - df['volume_sma']) / df['volume'].rolling(window=window).std()

        # Active trading features (BU/SD from FiinQuantX)
        if 'bu' in df.columns and 'sd' in df.columns:
            df['bu_sd_ratio'] = df['bu'] / \
                (df['sd'] + 1e-8)  # Avoid division by zero
            df['net_active_volume'] = df['bu'] - df['sd']
            df['active_volume_ratio'] = (
                df['bu'] - df['sd']) / (df['bu'] + df['sd'] + 1e-8)
    ...
    def engineer_features(
        ...
        if 'ticker' in data.columns:
            # Process by ticker
            result = data.groupby(
                'ticker', group_keys=False).apply(process_ticker_data)
        else:
            # Single ticker
        ...
        logger.info(
            f"Feature engineering complete: {original_cols} -> {new_cols} columns")
