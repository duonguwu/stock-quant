...
async def test_feature_engine():
    ...
    try:
        ...
        print(f"📝 Columns: {list(features.columns)[:10]}...")  # First 10 columns
...
async def test_backtest_integration():
    ...
    try:
        ...
        for signal in signals:
            print(f"🎯 {signal['ticker']}: {signal['action']} (conf: {signal['confidence']:.3f})")
...
async def test_signal_engine():
    ...
    try:
        ...
        for strategy_id, strategy in strategies.items():
            print(f"🎯 {strategy_id}: {strategy['name']} (conf: {strategy['confidence_threshold']})")
        ...
        if signal_engine.backtest_integration:
            ...
            if signals:
                ...
                for signal in signals[:3]:  # Test first 3 signals
                    print(f"📊 Processing {signal['ticker']}: {signal['action']} ({signal['confidence']:.3f})")
                    
                    # Simulate portfolio update (mock)
                    print(f"💼 Portfolio would be updated for {signal['ticker']}")
                    
                    # Simulate Telegram alert (mock)
                    if signal['confidence'] > 0.65:
                        print(f"📱 Telegram alert would be sent for {signal['ticker']}")
