                ...
                try:
                    ...
                    for callback in self.signal_callbacks:
                        try:
                            ...
                            logger.error(f"❌ Signal callback error: {cb_error}")
                    
                    logger.info(f"📡 Generated realtime signals for {len(tickers_list)} tickers")
...
def print_signals(signals: Dict):
    ...
    try:
        if signals.get('status') != 'success':
            print(f"❌ Signal generation failed: {signals.get('error', 'Unknown error')}")
            return
        ...
        for conf_key, conf_data in predictions.items():
            ...
            active_signals = [s for s in signals_list if s.get('action') != 'HOLD']
