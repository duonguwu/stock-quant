...
class SimpleFeatureEngine:
    ...
    def add_technical_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            plus_di = 100 * \
                (pd.Series(plus_dm).rolling(window=period).sum() / tr_sum)
            minus_di = 100 * \
                (pd.Series(minus_dm).rolling(window=period).sum() / tr_sum)
            ...
            if 'volume' in df.columns:
                ...
                obv_values = np.where(
                    price_change > 0, df['volume'], np.where(
                        price_change < 0, -df['volume'], 0))
    ...
    def engineer_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Feature engineering: {original_cols} -> {new_cols} columns")
