...
def generate_mock_bar(ticker: str, base_price: float = 25000):
    ...
    total_active = volume * random.uniform(0.3, 0.7)  # 30-70% of volume is active
...
async def test_signal_generation():
    ...
    if os.path.exists(ref_path):
        ...
        exclude_cols = ['label', 'hit_time', 'hit_type', 'ub', 'lb', 'vbar_end']
        ref_feature_cols = [col for col in test_ref.columns if col not in exclude_cols]
        ...
        if extra_features:
            ...
            for feat in sorted(extra_features):
                if feat not in ['ticker', 'timestamp', 'open', 'high', 'low', 'close', 'volume']:
                    print(f"  + {feat}")
    else:
        ...
        while True:
            ...
            for ticker in tickers:
                ...
                for i in range(100):
                    ...
                    bar['timestamp'] = datetime.now() - timedelta(minutes=15*(100-i))
                    historical_data.append(bar)
            ...
            for level, data in predictions.items():
                if level == 'conf_0.6':  # Show only 60% confidence for brevity
                    active_signals = [s for s in data['signals'] if s['action'] != 'HOLD']
                    if active_signals:
                        for signal in active_signals:
                            print(f"  📊 {signal['ticker']}: {signal['action']} "
                                  f"(confidence: {signal['confidence']:.2f})")
