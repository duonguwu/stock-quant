...
try:
    sys.path.append('/media/duongn/New Volume/UIT/AI Challenge/DATA DSTCDSTC/stock-quant/src')
...
class SimpleFeatureEngine:
    ...
    def __init__(self):
        ...
        try:
            if 'OriginalFeatureEngineer' in globals():
                ...
                class MockIndicator:
                    def ema(self, series, window): return series.ewm(span=window).mean()
                    def sma(self, series, window): return series.rolling(window).mean()
                    def rsi(self, series, window): return self._calculate_rsi(series, window)
                    def macd(self, series, **kwargs): return series.ewm(span=12).mean() - series.ewm(span=26).mean()
                    def macd_signal(self, series, **kwargs): return self.macd(series).ewm(span=9).mean()
                    def macd_diff(self, series, **kwargs): return self.macd(series) - self.macd_signal(series)
                    def atr(self, high, low, close, window): return self._calculate_atr(high, low, close, window)
                    ...
                    def bollinger_lband(self, series, window, window_dev):
                        ...
                        std = series.rolling(window).std()
                        return sma - (std * window_dev)
                    def stoch(self, high, low, close, window): return self._calculate_stoch(high, low, close, window)
                    def stoch_signal(self, high, low, close, window): return self.stoch(high, low, close, window).rolling(3).mean()
                    def adx(self, high, low, close, window): return pd.Series(50, index=close.index)  # Mock ADX
                    def mfi(self, high, low, close, volume, window): return pd.Series(50, index=close.index)  # Mock MFI
                    def vwap(self, high, low, close, volume, window): return (close * volume).rolling(window).sum() / volume.rolling(window).sum()
                    def obv(self, close, volume): return (volume * close.diff().apply(lambda x: 1 if x > 0 else -1 if x < 0 else 0)).cumsum()
    ...
    def process_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.use_original and self.original_engine:
            try:
                ...
                features_df = self.original_engine.engineer_features(data, config)
                print(f"✅ Original engine: {len(data.columns)} → {len(features_df.columns)} columns")
                return features_df
            except Exception as e:
    ...
    def _create_simplified_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'ticker' in df.columns:
            result = df.groupby('ticker', group_keys=False).apply(self._add_simple_features)
        else:
            result = self._add_simple_features(df)
            
        print(f"✅ Simplified features: {len(data.columns)} → {len(result.columns)} columns")
        return result
    ...
    def _add_simple_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'volume' in df.columns:
            ...
            df['volume_zscore'] = (df['volume'] - df['volume_sma']) / df['volume'].rolling(36).std()
    ...
    def prepare_for_inference(self, features_df: pd.DataFrame) -> pd.DataFrame:
        ...
        exclude_cols = ['ticker', 'timestamp', 'open', 'high', 'low', 'close', 'volume']
        ...
        print(f"📊 Prepared inference data: {inference_data.shape[0]} rows, {inference_data.shape[1]} features")
