...
class RealModelInference:
    ...
    def _find_model_path(self) -> str:
        ...
        raise FileNotFoundError(
            "❌ No model found. Please ensure model exists.")
    ...
    def predict_with_confidence(self,
        ...
        logger.info(
            f"📊 Generated predictions for {len(self.confidence_levels)} confidence levels")
        return results
    ...
    def _calculate_summary(self, signals: List[Dict]) -> Dict:
        ...
        avg_confidence = np.mean(
            [s['confidence'] for s in meeting_threshold]) if meeting_threshold else 0.0
