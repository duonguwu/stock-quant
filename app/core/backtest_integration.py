...
class MockBacktestEngine:
    ...
    def generate_signals(self, data: pd.DataFrame, confidence_threshold: float = 0.65) -> pd.DataFrame:
        """Generate mock signals"""
        n_rows = len(data)
        
        # Generate realistic signal distribution
        signals = np.random.choice([-1, 0, 1], size=n_rows, p=[0.15, 0.7, 0.15])
        confidence = np.random.beta(2, 5, size=n_rows) * 0.5 + 0.5  # Bias toward higher confidence
        ...
        try:
            if BacktestEngine15m:
                ...
                if os.path.exists(model_path) and os.path.exists(scaler_path):
                    self.backtest_engine = BacktestEngine15m(model_path, scaler_path)
                    logger.info("✅ Real backtest engine initialized")
                else:
                    logger.warning("⚠️ Model files not found, using mock engine")
                    self.backtest_engine = MockBacktestEngine()
            else:
                logger.warning("⚠️ BacktestEngine15m not available, using mock")
                self.backtest_engine = MockBacktestEngine()
    ...
    async def generate_realtime_signals(
        ...
        try:
            ...
            if features_df.empty:
                logger.warning("⚠️ No features available for signal generation")
                return []
    ...
    def _predict_with_model(
        ...
        try:
            ...
            return {
                ...
                'prob_hold': float(probabilities[1]) if len(probabilities) > 1 else 0,
                'prob_buy': float(probabilities[2]) if len(probabilities) > 2 else 0,
            }
