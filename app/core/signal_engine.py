...
class RealTimeSignalEngine:
    ...
    async def _engineer_features(self, historical_data: pd.DataFrame) -> Optional[np.ndarray]:
        ...
        try:
            ...
            # Volume indicators
            df['bu_sd_ratio'] = df['bu'] / (df['sd'] + 1)  # Avoid division by zero
        ...
        try:
            ...
            if confidence >= strategy.confidence_threshold and action != "HOLD":
                signal = {
                    ...
                    "strategy": strategy.name,
                    "strategy_id": list(self.strategies.keys())[list(self.strategies.values()).index(strategy)],
                ...
                logger.info(f"🎯 Signal: {ticker} {action} (conf: {confidence:.3f}) - {strategy.name}")
                return signal
