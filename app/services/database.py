...
class MongoDBService:
        ...
        try:
            ...
            try:
                ...
                if "authentication" in str(auth_error).lower():
                    logger.warning("⚠️ MongoDB requires authentication, using no-auth mode for development")
                    # For development, use MongoDB without authentication
                    self.client = AsyncIOMotorClient("mongodb://localhost:27017/?authSource=admin")
                    self.db = self.client[self.settings.mongodb_database]
                    await self.db.command("ping")
    ...
    async def save_signal(self, signal_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.debug(f"💾 Signal saved: {signal_data['ticker']} {signal_data['action']}")
    ...
    async def get_recent_signals(self, limit: int = 50, strategy: str = None) -> List[Dict]:
        ...
        try:
            ...
            cursor = self.db.signals.find(query).sort("timestamp", -1).limit(limit)
    ...
    async def save_portfolio_snapshot(self, account_id: str, portfolio_data: Dict[str, Any]) -> str:
    ...
    async def get_portfolio_summary(self, account_id: str = "demo") -> Dict[str, Any]:
    ...
    async def cleanup_old_data(self, days: int = 30):
        ...
        try:
            ...
            logger.info(f"🧹 Cleaned up old data: {result1.deleted_count + result2.deleted_count + result3.deleted_count} records")
