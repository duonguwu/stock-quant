...
class FeatureEngineer:
    ...
    def engineer_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Feature engineering: {len(data.columns)} -> {len(result.columns)} columns")
        return result
    ...
    def _add_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        minus_di = 100 * \
            (pd.Series(minus_dm).rolling(window=14).sum() / tr_sum)
            ...
            for conf_threshold in self.confidence_levels:
                ...
                for i, ticker in enumerate(tickers):
                    ...
                    if confidence >= conf_threshold:
                        action = {-1: "SELL", 0: "HOLD",
                                  1: "BUY"}[original_signal]
                    else:
                        action = "HOLD"
                ...
                # Calculate summary
                active_signals = [
                    s for s in level_signals if s['action'] != 'HOLD']
                buy_signals = [
                    s for s in level_signals if s['action'] == 'BUY']
                sell_signals = [
                    s for s in level_signals if s['action'] == 'SELL']
...
def format_signals_output(results: Dict, show_details: bool = True) -> str:
    ...
    for conf_key, conf_data in results.items():
        ...
        output.append(
            f"   Active: {summary['active']}/{summary['total']} signals")
        output.append(
        ...
        if show_details:
            ...
            for signal in active_signals:
                ...
                output.append(
                    f"   {action_emoji} {signal['ticker']}: {signal['action']} "
