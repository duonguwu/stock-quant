...
# --------- DANH SÁCH MÃ ----------
CORE_TICKERS = ['CTG', 'MBB', 'ACB', 'QNS',
                'MSH']              # theo dõi thường trực
OPTIONAL_TICKERS = [
    'HDB',
    'STB',
    'TPB',
...
class SlidingWindowStreamer:
    ...
    def __init__(
        ...
        self.buffers: Dict[str, deque] = {
            t: deque(maxlen=self.window_bars) for t in self.tickers}
    ...
    def _on_update(self, data: BarDataUpdate):
        ...
        for tkr, g in df.groupby('ticker', sort=False):
            ...
            # Gọi callback do người dùng định nghĩa (ví dụ: tính RSI/VSA → phát
            # tín hiệu)
            try:
    ...
    def _print_window(df_window: pd.DataFrame):
        ...
        print(
            f"[{tkr}] window({len(df_window)}): đến {last_ts} | close={df_window['close'].iloc[-1]}")

    # ---- BẮT ĐẦU STREAM ----
    def start(self):
        print(
            f"🚀 Start stream: {self.tickers} | by={self.by} | init_period={self.init_period} | window={self.window_bars}")
...
def start_optional_stream(
    ...
    tickers = (
        CORE_TICKERS +
        OPTIONAL_TICKERS) if include_core else OPTIONAL_TICKERS
    streamer = SlidingWindowStreamer(
        tickers=tickers,
        by=by,
        init_period=init_period,
